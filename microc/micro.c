...
__inline struct variable *
context_varadd(struct context *ctx, const char *const type, const char *const name, const char *const value,
               int external) {
    if (ctx->vars == NULL && ctx->varc == 0) {
        ctx->vars = (struct variable *) malloc(sizeof(struct variable));
    } else {
        ctx->vars = (struct variable *) realloc(ctx->vars,
                                                (ctx->varc * sizeof(struct variable)) + sizeof(struct variable));
    }
...
context_typeadd(struct context *ctx, const char *const name, int pointer) {
    if (ctx->decs == NULL && ctx->decc == 0) {
        ...
        ctx->decs = (struct decl_type *) realloc(ctx->decs,
                                                 (ctx->decc * sizeof(struct decl_type)) + sizeof(struct decl_type));
    }
...
context_funadd(struct context *ctx, const char *const name, const char *const type, int pointer, int isunsigned) {
    if (ctx->funs == NULL && ctx->func == 0) {
        ...
        ctx->funs = (struct function *) realloc(ctx->funs,
                                                (ctx->func * sizeof(struct function)) + sizeof(struct function));
    }
...
context_funaddparm(struct function *fun, const char *const type, const char *const name, int pointer, int isunsigned) {
    if (fun->parms == NULL && fun->parmc == 0) {
        ...
        fun->parms = (struct fun_parm *) realloc(fun->parms,
                                                 (fun->parmc * sizeof(struct fun_parm)) + sizeof(struct fun_parm));
    }
...
expr0(struct lexer *lexer, struct codegen *codegen) {
    ...
    if (lexer_match(lexer, "+") || lexer_match(lexer, "-") || lexer_match(lexer, "/") || lexer_match(lexer, "%") ||
        lexer_match(lexer, "*")) {
